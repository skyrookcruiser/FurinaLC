...
async def handle(request: Request):
    ...
    if req_input.startswith("GIVEALL"):
        res_update, res_packet = give_all()
    elif req_input.startswith("EGO"):
        res_update, res_packet = parse_ego(req_input)
    elif req_input.startswith("PER"):
        res_update, res_packet = parse_personality(
            req_input
        )
